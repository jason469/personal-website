...
export default function VolunteerCard({ data }: volunteeringCardProp) {
  ...
  return (
    <div>
      {showDescriptionHeight && (
        <Card
          className={`${styles.card}`}
          id={`volunteerCard__${data.company}__hideCard`}
        >
          <div id={`volunteerCard_${data.company}__hide`}>
            <div ref={hiddenDescription}>
      ...
      <Card className={`${styles.card}`}>
        ...
        <CardActions disableSpacing className={`${styles.card__actions}`}>
          <ExpandMore
            expand={expanded}
            onClick={expandCard}
          ...
          >
            <FontAwesomeIcon icon={faCaretDown} className={`p-0`} />
          </ExpandMore>
        </CardActions>
        <AnimateHeight duration={500} height={height}>
          <Collapse in={expanded} unmountOnExit timeout={30}>
            <CardContent>
